-- =====================================================
-- SCHEMA FINOPS/GREENOPS AVANCÉ - GCP (VERSION SAFE)
-- Tables optimisées pour analyses de coûts et performance
-- Version qui gère les éléments existants
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'gcp_projects_user_id_project_id_key'
  ) THEN
    ALTER TABLE gcp_projects ADD CONSTRAINT gcp_projects_user_id_project_id_key UNIQUE(user_id, project_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'gcp_services_usage_user_id_project_id_service_id_billing_p_key'
  ) THEN
    ALTER TABLE gcp_services_usage ADD CONSTRAINT gcp_services_usage_user_id_project_id_service_id_billing_p_key UNIQUE(user_id, project_id, service_id, billing_period, sku_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'gcp_budgets_tracking_user_id_project_id_budget_name_period_key'
  ) THEN
    ALTER TABLE gcp_budgets_tracking ADD CONSTRAINT gcp_budgets_tracking_user_id_project_id_budget_name_period_key UNIQUE(user_id, project_id, budget_name, period_start);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'gcp_carbon_footprint_user_id_project_id_measurement_perio_key'
  ) THEN
    ALTER TABLE gcp_carbon_footprint ADD CONSTRAINT gcp_carbon_footprint_user_id_project_id_measurement_perio_key UNIQUE(user_id, project_id, measurement_period, region);
  END IF;
END $$;

-- =====================================================
-- INDEXES POUR PERFORMANCE (CREATE IF NOT EXISTS)
-- =====================================================

-- gcp_projects
CREATE INDEX IF NOT EXISTS idx_gcp_projects_user_id ON gcp_projects(user_id);
CREATE INDEX IF NOT EXISTS idx_gcp_projects_sync_status ON gcp_projects(sync_status);
CREATE INDEX IF NOT EXISTS idx_gcp_projects_cost ON gcp_projects(current_month_cost DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_projects_carbon ON gcp_projects(carbon_footprint_kg DESC);

-- gcp_services_usage
CREATE INDEX IF NOT EXISTS idx_gcp_services_user_project ON gcp_services_usage(user_id, project_id);
CREATE INDEX IF NOT EXISTS idx_gcp_services_period ON gcp_services_usage(billing_period DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_services_cost ON gcp_services_usage(cost_amount DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_services_category ON gcp_services_usage(service_category);
CREATE INDEX IF NOT EXISTS idx_gcp_services_location ON gcp_services_usage(region, zone);

-- gcp_cost_anomalies
CREATE INDEX IF NOT EXISTS idx_gcp_anomalies_user_project ON gcp_cost_anomalies(user_id, project_id);
CREATE INDEX IF NOT EXISTS idx_gcp_anomalies_severity ON gcp_cost_anomalies(severity, detected_at DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_anomalies_status ON gcp_cost_anomalies(status);

-- gcp_budgets_tracking
CREATE INDEX IF NOT EXISTS idx_gcp_budgets_user_project ON gcp_budgets_tracking(user_id, project_id);
CREATE INDEX IF NOT EXISTS idx_gcp_budgets_utilization ON gcp_budgets_tracking(utilization_percent DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_budgets_period ON gcp_budgets_tracking(period_start, period_end);

-- gcp_optimization_recommendations
CREATE INDEX IF NOT EXISTS idx_gcp_recommendations_user_project ON gcp_optimization_recommendations(user_id, project_id);
CREATE INDEX IF NOT EXISTS idx_gcp_recommendations_priority ON gcp_optimization_recommendations(priority, potential_monthly_savings DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_recommendations_status ON gcp_optimization_recommendations(status);
CREATE INDEX IF NOT EXISTS idx_gcp_recommendations_category ON gcp_optimization_recommendations(category);

-- gcp_carbon_footprint
CREATE INDEX IF NOT EXISTS idx_gcp_carbon_user_project ON gcp_carbon_footprint(user_id, project_id);
CREATE INDEX IF NOT EXISTS idx_gcp_carbon_period ON gcp_carbon_footprint(measurement_period DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_carbon_emissions ON gcp_carbon_footprint(total_emissions_kg DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_carbon_region ON gcp_carbon_footprint(region);
...
-- Appliquer les triggers seulement s'ils n'existent pas
DO $$ 
BEGIN
...
-- Index pour l'audit
CREATE INDEX IF NOT EXISTS idx_gcp_audit_user_id ON gcp_audit_log(user_id);
CREATE INDEX IF NOT EXISTS idx_gcp_audit_action ON gcp_audit_log(action, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_gcp_audit_created_at ON gcp_audit_log(created_at DESC);
...
BEGIN
  -- gcp_services_usage
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gcp_services_usage' AND column_name = 'archived_at') THEN
    ALTER TABLE gcp_services_usage ADD COLUMN archived_at TIMESTAMP WITH TIME ZONE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gcp_services_usage' AND column_name = 'is_archived') THEN
    ALTER TABLE gcp_services_usage ADD COLUMN is_archived BOOLEAN DEFAULT false;
  END IF;
  
  -- gcp_projects
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gcp_projects' AND column_name = 'archived_at') THEN
    ALTER TABLE gcp_projects ADD COLUMN archived_at TIMESTAMP WITH TIME ZONE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gcp_projects' AND column_name = 'disconnection_reason') THEN
    ALTER TABLE gcp_projects ADD COLUMN disconnection_reason TEXT;
  END IF;
END $$;
...
DO $$ 
BEGIN
  RAISE NOTICE '🎉 Schema GreenOps FinOps installé avec succès !';
  RAISE NOTICE '📊 Tables créées : gcp_projects, gcp_services_usage, gcp_cost_anomalies, gcp_budgets_tracking, gcp_optimization_recommendations, gcp_carbon_footprint';
  RAISE NOTICE '🔍 Vues créées : v_finops_dashboard, v_cost_by_service';
  RAISE NOTICE '⚡ Index et triggers configurés pour les performances';
  RAISE NOTICE '✅ Prêt pour la synchronisation GCP !';
END $$;
